import{d as T,g as $}from"./main.9c82b603.js";import"./wScoge.fc00b881.js";const f="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",V=new T.Configuration({apiKey:f}),x=new T.OpenAIApi(V);var u=null,s=null,e=null;async function M(B){try{const c=await fetch("/src/sudb.json");if(!c.ok)throw new Error(`HTTP error! Status: ${c.status}`);u=await c.json(),s=u.SUD.Scenarios.Intro}catch(c){console.error("Error fetching JSON data:",c)}if(B===!1){const c=document.createElement("link");c.rel="stylesheet",c.href="editor.css",document.head.appendChild(c);var v=document.createElement("div"),A=document.getElementById("main");v.setAttribute("id","editorScreen"),v.innerHTML=`
            <div id="editorHeader">
